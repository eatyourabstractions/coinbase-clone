...
const CoinItem = ({
  ...
  useEffect(() => {
    const getBalance = async () => {
      ...
      twTokens.map(async twToken => {
        if (twToken.address === token.contractAddress) {
          // activeTwToken = twToken
          
          const balance = await twToken.contractWrapper.readContract.balanceOf(walletAddress)
          console.log('activeTwToken balance', parseInt(balance) / 10 ** 18)
