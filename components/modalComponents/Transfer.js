...
const Transfer = ({ setAction, twTokens, selectedToken, walletAddress }) => {
  ...
  useEffect(() => {
    twTokens.map(token => {
      if (token.contractWrapper.readContract.address === selectedToken.contractAddress) {
  ...
  useEffect(() => {
    const getBalance = async () => {
      ...
      // console.log('activeTwToken  Transfer', activeTwToken)
  ...
  const sendCrypto = async () => {
    ...
    if (activeTwToken && amount && recipient) {
      ...
      console.log('sendCrypto result', result)
