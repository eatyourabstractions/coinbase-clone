...
const Portfolio = ({ twTokens, sanityTokens, walletAddress }) => {
  ...
  const getBalance = async activeTwToken => {
    const balance = await activeTwToken.contractWrapper.readContract.balanceOf(walletAddress)
    // console.log('getBalance--address',walletAddress)
    // console.log('getBalance ayo', parseInt(balance) / 10 ** 18)
    ...
    const calculateTotalBalance = async () => {
      ...
      sanityTokens.map(async token => {
        const currentTwToken = twTokens.filter(
          twToken => twToken.contractWrapper.readContract.address === token.contractAddress,
        )
        // console.log('Portfolio.js: currentTwToken', currentTwToken)

        const balance = await getBalance(currentTwToken[0])
        balanceList.push(balance * token.usdPrice)
        setWalletBalance(prevState => prevState + balance * token.usdPrice)
        // console.log('Portfolio.js: balanceList', balanceList)
      })
